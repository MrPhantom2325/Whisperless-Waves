...
function IntroScreen({ onStart }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, color: "#2A2010", letterSpacing: 2 }}>PHASE 3 — SIX VOYAGE STAGES</div>
    </div>
  );
}
...
export default function BlindCaptainGame() {
  const [screen, setScreen] = useState("intro");       // intro | stageIntro | playing | stageComplete | gameover
  ...
  if (screen === "victory") {
    return (
      <div style={{
        ...
        <div style={{ position: "absolute", inset: 24, border: "1px solid #5C4A1E", pointerEvents: "none" }} />
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 10, letterSpacing: 5, color: "#8B7340", marginBottom: 16 }}>THE VOYAGE IS COMPLETE</div>
          <div style={{
            fontSize: 38, color: "#C8961E", letterSpacing: 2, marginBottom: 10,
            textShadow: "0 0 40px rgba(200,150,30,0.5)",
          }}>
            Isla del Tesoro
          </div>
          <div style={{ fontSize: 13, color: "#5C4A1E", letterSpacing: 1, marginBottom: 24, lineHeight: 1.8 }}>
            The treasure of Captain Blackwave Voss is yours.
          </div>
          <div style={{
            border: "1px solid #3A2E12", padding: "16px 28px", maxWidth: 440,
            fontSize: 11, color: "#3A2E12", lineHeight: 1.9, textAlign: "left",
          }}>
            ...
            <span style={{ color: "#5C4A1E" }}>The saga is complete.</span>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {STAGES.map(s => (
            <div key={s.id} style={{
              ...
              background: "#D4A420", border: "1px solid #5C4A1E",
              boxShadow: "0 0 6px rgba(212,164,32,0.6)",
            }} />
          ))}
        </div>
        <button
          onClick={() => { setStageIdx(0); setScreen("intro"); }}
          style={{
