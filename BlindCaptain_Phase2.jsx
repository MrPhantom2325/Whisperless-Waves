...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const GRID_SIZE = 12;
const TICK_MS   = 600; // ship movement tick interval
...
/** Given heading degrees, returns [dx, dy] step */
function headingToDelta(heading) {
  const rad = (heading * Math.PI) / 180;
  const dx  = Math.round(Math.sin(rad)); // E positive
  const dy  = Math.round(-Math.cos(rad)); // N negative (grid y goes down)
  return [dx, dy];
}
...
// ═════════════════════════════════════════════════════════════════════════════
//  CAPTAIN'S PANEL — sees the map; WASD to move cursor; Space to ping
// ═════════════════════════════════════════════════════════════════════════════
function CaptainPanel({ ship, pingCell, cursorPos, dangerPing }) {
  const CELL = 48;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 10, color: "#5C4A1E", fontFamily: "'Courier New', monospace" }}>
          [W A S D] move cursor · [SPACE] ping
        </span>
      </div>
          ...
          <div style={{
            ...
            {Array.from({ length: GRID_SIZE }, (_, row) =>
              Array.from({ length: GRID_SIZE }, (_, col) => {
                ...
                const isCursor = cursorPos.x === col && cursorPos.y === row && !isShip;
                const isDanger = dangerPing === key;
                ...
                return (
                  <div key={key} style={{
                    ...
                    boxShadow: isPing
                      ? "inset 0 0 12px rgba(255,200,50,0.6)"
                      : isDanger
                        ? "inset 0 0 14px rgba(255,60,60,0.7)"
                        : "none",
                  }}>
                    {/* Cell icon */}
                    {type !== "sea" && (
                    ...
                    {/* Captain's cursor (crosshair) */}
                    {isCursor && (
                      <div style={{
                        position: "absolute", inset: 2,
                        border: "1px dashed rgba(212,164,32,0.5)",
                        borderRadius: 1,
                        pointerEvents: "none",
                      }} />
                    )}
...
// ═════════════════════════════════════════════════════════════════════════════
//  NAVIGATOR'S PANEL — controls the ship; Arrow keys, Q/E
// ═════════════════════════════════════════════════════════════════════════════
function NavigatorPanel({ ship, onSteer, onSails, onAnchor, log }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, height: 1, background: "linear-gradient(90deg, #5C4A1E, transparent)" }} />
        <span style={{ fontSize: 10, color: "#5C4A1E", fontFamily: "'Courier New', monospace" }}>
          [←→] steer · [↑↓] sails · [Q] anchor
        </span>
      </div>

      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "16px 24px", gap: 16, zIndex: 2, overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            ...
            <span style={{ fontSize: 16, color: "#D4A420", fontFamily: "'Courier New', monospace" }}>{ship.sails}%</span>
          </div>
          <div style={{
            ...
            <div style={{
              ...
              boxShadow: ship.sails > 0 ? "2px 0 8px rgba(212,164,32,0.4)" : "none",
            }} />
            {[25,50,75].map(t => (
              <div key={t} style={{
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
            <span style={{ fontSize: 9, letterSpacing: 3, color: "#5C4A1E", fontFamily: "'Courier New', monospace" }}>HULL INTEGRITY</span>
        ...
        {/* ── SHIP POSITION READOUT (new in Phase 2) ── */}
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <span style={{ fontSize: 9, letterSpacing: 3, color: "#5C4A1E", fontFamily: "'Courier New', monospace" }}>POSITION</span>
          <div style={{
            padding: "6px 10px",
            background: "#080502",
            ...
            <span style={{ fontSize: 11, color: "#8B7340", fontFamily: "'Courier New', monospace" }}>
              COL {String(ship.x).padStart(2,"0")} · ROW {String(ship.y).padStart(2,"0")}
            </span>
            <span style={{ fontSize: 11, color: "#5C4A1E", fontFamily: "'Courier New', monospace" }}>
              {MAP_CELLS[`${ship.x},${ship.y}`]?.toUpperCase() ?? "OPEN SEA"}
            </span>
          </div>
        </div>

        {/* ── SHIP'S LOG (new in Phase 2) ── */}
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <span style={{ fontSize: 9, letterSpacing: 3, color: "#5C4A1E", fontFamily: "'Courier New', monospace" }}>SHIP'S LOG</span>
          <div ref={logRef} style={{
            ...
            {log.map((entry, i) => (
              <span key={i} style={{
                fontSize: 9,
                color: entry.type === "danger" ? "#CC3333"
                     : entry.type === "event"  ? "#C8961E"
                     : "#5C4A1E",
                fontFamily: "'Courier New', monospace",
                letterSpacing: 0.5,
              }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", maxWidth: 560 }}>
        <div style={{ fontSize: 11, letterSpacing: 6, color: "#5C4A1E", marginBottom: 16 }}>VOYAGE 01</div>
        <h1 style={{
          fontSize: 42, fontWeight: "normal", color: "#C8961E",
          lineHeight: 1.2, margin: "0 0 8px",
          textShadow: "0 0 40px rgba(200,150,30,0.4)", letterSpacing: 2,
        }}>The Blind Captain</h1>
        <h1 style={{
          fontSize: 42, fontWeight: "normal", color: "#8B7340",
          lineHeight: 1.2, margin: "0 0 24px", letterSpacing: 2,
        }}>& The Mute Navigator</h1>
        <p style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
        <div style={{
          ...
          {[
            ["CAPTAIN",   "Left side",  "Sees the map, hazards & destination",  "WASD move cursor · SPACE ping"],
            ["NAVIGATOR", "Right side", "Controls helm, sails & anchor",         "← → steer · ↑ ↓ sails · Q anchor"],
        ...
        <button
          ...
          style={{
            ...
            background: "transparent", border: "1px solid #C8961E",
            color: "#C8961E", fontFamily: "'Courier New', monospace",
            fontSize: 12, letterSpacing: 4, cursor: "pointer", transition: "all 0.2s",
          }}
      ...
      <div style={{ fontSize: 9, color: "#2A2010", letterSpacing: 2 }}>
        PHASE 2 — KEYBOARD INPUT & SHIP LOGIC
      </div>
    </div>
  );
}
...
function GameOverScreen({ won, onRestart }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, letterSpacing: 6, color: won ? "#C8961E" : "#8B2020" }}>
        {won ? "TREASURE FOUND" : "SHIP LOST"}
      </div>
      <h2 style={{
        fontSize: 36, fontWeight: "normal",
        color: won ? "#D4A420" : "#CC3333",
        textShadow: won ? "0 0 40px rgba(212,164,32,0.5)" : "0 0 40px rgba(200,0,0,0.5)",
        margin: 0, letterSpacing: 2,
      }}>
        {won ? "Isla del Tesoro reached!" : "The sea claims another..."}
      </h2>
      <button
        onClick={onRestart}
        style={{
          padding: "12px 40px", background: "transparent",
          border: `1px solid ${won ? "#C8961E" : "#8B2020"}`,
          color: won ? "#C8961E" : "#CC3333",
          fontFamily: "'Courier New', monospace",
...
export default function BlindCaptainGame() {
  ...
  const [dangerPing, setDangerPing] = useState(null);
  const [log,        setLog]        = useState([{ msg: "Voyage begins. The Captain watches the charts.", type: "event" }]);
  ...
  // ── Hazard flash on captain's map ─────────────────────────────────────────
  const flashDanger = useCallback((key) => {
    setDangerPing(key);
    clearTimeout(dangerTimer.current);
    dangerTimer.current = setTimeout(() => setDangerPing(null), 1500);
  }, []);

  // ── Movement tick — ship advances one cell in heading direction ───────────
  useEffect(() => {
    if (!started || gameOver) return;
    tickRef.current = setInterval(() => {
      setShip(prev => {
        if (prev.speed === 0 || prev.anchor) return prev;

        const [dx, dy] = headingToDelta(prev.heading);

        // Move speed times per TICK_MS would be too fast; we advance 1 cell per tick
        // but only when speed > 0
        const nx = Math.max(0, Math.min(GRID_SIZE - 1, prev.x + dx));
        const ny = Math.max(0, Math.min(GRID_SIZE - 1, prev.y + dy));
        const cellType = MAP_CELLS[`${nx},${ny}`] || "sea";

        // ── Collision logic ──
        if (cellType === "reef" || cellType === "island") {
          const newHP = prev.hullHP - 1;
          // Flash on map for captain
          flashDanger(`${nx},${ny}`);
          addLog(`💥 COLLISION! ${cellType === "reef" ? "Reef" : "Island"} at [${nx},${ny}] — hull struck!`, "danger");
          if (newHP <= 0) {
            addLog("⚠ Hull breached. The ship is lost.", "danger");
            setTimeout(() => setGameOver("lost"), 400);
          }
          // Bounce back: don't move, just lose HP
          return { ...prev, hullHP: newHP };
        }

        if (cellType === "current") {
          // Current pushes ship 1 extra cell in same direction
          addLog(`〜 Strong current at [${nx},${ny}] — carried forward!`, "info");
        }

        if (cellType === "destination") {
          addLog("✦ ISLA DEL TESORO! We've arrived!", "event");
          setTimeout(() => setGameOver("won"), 500);
        }

        return { ...prev, x: nx, y: ny };
      });
    }, TICK_MS * (4 - (1))); // base tick; speed multiplier handled below
    return () => clearInterval(tickRef.current);
  }, [started, gameOver, flashDanger, addLog]);

  // ── Speed-based tick rate: faster ticks when speed is higher ─────────────
  useEffect(() => {
    if (!started || gameOver) return;
    clearInterval(tickRef.current);
    const interval = ship.speed === 0 ? null
                   : ship.speed === 1 ? 1800
                   : ship.speed === 2 ? 1100
                   :                    650;
    if (!interval) return;
    tickRef.current = setInterval(() => {
      setShip(prev => {
        ...
        if (cellType === "reef" || cellType === "island") {
          const newHP = prev.hullHP - 1;
          flashDanger(`${nx},${ny}`);
          addLog(`💥 ${cellType === "reef" ? "Reef" : "Island"} at [${nx},${ny}] — hull struck!`, "danger");
          if (newHP <= 0) {
            addLog("⚠ Hull breached. The ship is lost.", "danger");
            setTimeout(() => setGameOver("lost"), 400);
          }
        ...
        if (cellType === "current") {
          addLog(`〜 Current at [${nx},${ny}] — swept along.`, "info");
        }
    ...
    return () => clearInterval(tickRef.current);
  }, [ship.speed, ship.anchor, started, gameOver, flashDanger, addLog]);

  // ── Derive speed from sails + anchor ─────────────────────────────────────
  useEffect(() => {
    const sp = ship.anchor ? 0
             : ship.sails < 25 ? 0
             : ship.sails < 50 ? 1
             : ship.sails < 80 ? 2
             : 3;
  ...
  const handleKey = useCallback((e) => {
    ...
    // CAPTAIN: WASD = move cursor; Space = ping
    if (key === "w" || key === "W") setCursorPos(p => ({ ...p, y: Math.max(0, p.y - 1) }));
    if (key === "s" || key === "S") setCursorPos(p => ({ ...p, y: Math.min(GRID_SIZE-1, p.y + 1) }));
    if (key === "a" || key === "A") setCursorPos(p => ({ ...p, x: Math.max(0, p.x - 1) }));
    if (key === "d" || key === "D") setCursorPos(p => ({ ...p, x: Math.min(GRID_SIZE-1, p.x + 1) }));
    if (key === " ") {
      setCursorPos(p => {
        const k = `${p.x},${p.y}`;
        setPingCell(k);
        ...
        const cell = MAP_CELLS[k];
        addLog(`📍 Captain pings [${p.x},${p.y}]${cell ? ` — ${cell}` : ""}`, "info");
        return p;
      });
      e.preventDefault();
    }

    // NAVIGATOR: arrows = steer/sails; Q = anchor
    if (key === "ArrowLeft") {
      setShip(s => ({ ...s, heading: (s.heading - 15 + 360) % 360 }));
      addLog("⬅ Helm hard to port.", "info");
    }
    if (key === "ArrowRight") {
      setShip(s => ({ ...s, heading: (s.heading + 15) % 360 }));
      addLog("➡ Helm hard to starboard.", "info");
    }
    if (key === "ArrowUp") {
      setShip(s => {
        const ns = Math.min(100, s.sails + 10);
        addLog(`▲ Sails at ${ns}%`, "info");
        return { ...s, sails: ns };
      });
    }
    if (key === "ArrowDown") {
      setShip(s => {
        const ns = Math.max(0, s.sails - 10);
        addLog(`▼ Sails at ${ns}%`, "info");
        return { ...s, sails: ns };
      });
    }
    if (key === "q" || key === "Q") {
      setShip(s => {
        const next = !s.anchor;
        addLog(next ? "⚓ Anchor dropped." : "⚓ Anchor raised.", "info");
        return { ...s, anchor: next };
      });
    }

    if (["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"," "].includes(key)) e.preventDefault();
  }, [started, gameOver, addLog]);
  ...
  // ── Steer/sail button handlers ────────────────────────────────────────────
  const onSteer = useCallback((dir) => {
    setShip(s => ({
      ...s, heading: dir === "left"
  ...
  const onAnchor = useCallback(() => {
    setShip(s => ({ ...s, anchor: !s.anchor }));
  }, []);

  // ── Restart ───────────────────────────────────────────────────────────────
  const handleRestart = () => {
    setShip({ ...INITIAL_SHIP });
    setCursorPos({ x: 2, y: 2 });
    setPingCell(null);
    setDangerPing(null);
    setLog([{ msg: "New voyage. The sea awaits.", type: "event" }]);
    setGameOver(null);
  };
  ...
  return (
    <div style={{
      ...
      {gameOver && (
        <GameOverScreen won={gameOver === "won"} onRestart={handleRestart} />
      )}
    </div>
  );
}
